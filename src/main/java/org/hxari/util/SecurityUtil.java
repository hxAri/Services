...
public class SecurityUtil
{
	...
	{
		return( SecurityUtil.hash( password, SecurityUtil.salt() ) );
	}
	...
	{
		return( SecurityUtil.hash( password.toCharArray(), SecurityUtil.salt( length ) ) );
	}
	...
	{
		return( SecurityUtil.hash( password.toCharArray(), salt ) );
	}
	...
	{
		return( SecurityUtil.hash( password.toCharArray(), salt.getBytes() ) );
	}
	...
	{
		return( SecurityUtil.hash( SecretKeyFactory
			.getInstance( SecurityUtil.DEFAULT_HASH_ALGO )
			.generateSecret( 
				new PBEKeySpec( 
					password, 
					salt, 
					SecurityUtil.DEFAULT_HASH_ITER, 
					SecurityUtil.DEFAULT_HASH_LENGTH 
				) 
			)
			.getEncoded(), 
			salt 
	...
	{
		return( SecurityUtil.salt( SecurityUtil.DEFAULT_SALT_LENGTH ) );
	}
	...
	{
		return( SecurityUtil.valid( raw ) == optional );
	}
	...
	public static boolean valid( String raw )
	{
		try
		{
			...
			return( parts.length == 3 ? ( Integer.parseInt( parts[0] ) >= SecurityUtil.DEFAULT_HASH_ITER ) : false );
		}
		catch( Throwable e )
		{
			return( false );
	...
	public static boolean verify( String hashed, String password ) throws Exception
	{
		...
		if( parts.length == 3 )
		{
			...
			byte[] salt = HexUtil.decode( parts[1] );
			byte[] hash = HexUtil.decode( parts[2] );
